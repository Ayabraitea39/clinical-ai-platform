...
function OrderMedicalActModal({ classification, onClose, onSave }) {
  ...
  function handleSubmit(e) {
    ...
    onSave({
      ...
      medical_act_name: medicalAct.name,
      classification,
      reason,
      notes,
    });
...
function VisitFillPage() {
  ...
  useEffect(() => {
    async function load() {
      const [visitData, formData, existingSigns] = await Promise.all([getVisit(id), getSignForm(id), getVisitSigns(id)]);
      setVisit(visitData);
  ...
  function handleAddPrescription(prescription) { setPrescriptions((current) => [...current, prescription]); }
  function handleRemovePrescription(index) { setPrescriptions((current) => current.filter((_, i) => i !== index)); }
  ...
  function handleAddOrder(order) { setOrders((current) => [...current, order]); }
  function handleRemoveOrder(index) { setOrders((current) => current.filter((_, i) => i !== index)); }
  ...
  return (
    <div className="visit-card-page">
          ...
          <div className="entries-scroll">
            {prescriptions.map((prescription, index) => (
              <div className="history-entry" key={index}>
                ...
                <div style={{ marginTop: 8 }}><button type="button" onClick={() => handleRemovePrescription(index)} className="entry-remove-btn">Remove</button></div>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="patient-card-section">
        <h3>Medical Acts</h3>
        {orders.length === 0 ? <p className="empty-hint">No laboratory tests, imaging, or other medical acts added yet.</p> : (
          <div className="entries-scroll">
            {orders.map((order, index) => (
              <div className="history-entry" key={index}>
                <div className="field-label">{ORDER_TYPE_LABELS[order.classification]}</div>
                ...
                <div style={{ marginTop: 8 }}><button type="button" onClick={() => handleRemoveOrder(index)} className="entry-remove-btn">Remove</button></div>
              </div>
            ))}
          </div>
        )}
